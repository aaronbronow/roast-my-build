...
/**
 * Compute SHA256 of file for lockfile hash checking.
 */
...
async function main() {
  ...
  const lockfileHashBefore = getFileSha256(lockfilePath);

  // 3. Execute Build 1 (Standard) and measure duration & output logs
  console.log('Running standard Build 1...');
  const start1 = Date.now();
  let buildLog1 = '';
  let buildError1 = null;

  try {
    // Capture stdout/stderr of build
    buildLog1 = execSync(buildCmd, { env: process.env, encoding: 'utf8', maxBuffer: 50 * 1024 * 1024 });
    console.log(buildLog1);
  } catch (error) {
    buildLog1 = (error.stdout || '') + '\n' + (error.stderr || '');
    console.error(buildLog1);
    buildError1 = error;
  }
  const duration1 = Date.now() - start1;

  if (buildError1) {
    console.error('❌ Standard Build 1 failed execution. Aborting fitness test.');
    process.exit(1);
  }

  // Check if lockfile was mutated
  const lockfileHashAfter = getFileSha256(lockfilePath);
  const lockfileMutated = lockfileHashBefore !== '' && lockfileHashAfter !== '' && lockfileHashBefore !== lockfileHashAfter;
  if (lockfileMutated) {
    console.warn('⚠️ Warning: package-lock.json was modified during the build execution!');
  }
  ...
  // Restore package-lock.json if mutated
  if (lockfileMutated) {
    try {
  ...
  try {
    ...
    console.warn('⚠️ Mutated Build 2 failed execution. Proceeding to compare outputs.');
  }
